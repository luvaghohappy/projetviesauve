...
if (empty($noms) || empty($email) || empty($sexe) || empty($code) || empty($fonction) || empty($secteur_id) || empty($mot_de_passe)) {
...
$stmt = $connect->prepare("INSERT INTO administrateurs (noms, email, sexe, CODE, Fonction, secteur_id, mot_de_passe, image_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("ssssssss", $noms, $email, $sexe, $code, $fonction, $secteur_id, $hashed_password, $image_path);
